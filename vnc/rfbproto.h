...
/* TigerVNC protocol extensions (server -> client) */
#define rfbEndOfContinuousUpdates 150
#define rfbServerFence 248
...
/* TigerVNC protocol extensions (client -> server) */
#define rfbEnableContinuousUpdates 150
#define rfbClientFence 248
...
/* TigerVNC pseudo-encodings */
#define rfbEncodingDesktopName          0xFFFFFECD /* -307 */
...
/* ExtendedDesktopSize rect header: x = reason, y = result code */
...
/* Fence flags */
#define rfbFenceFlagBlockBefore (1<<0)
#define rfbFenceFlagBlockAfter  (1<<1)
#define rfbFenceFlagSyncNext    (1<<2)
#define rfbFenceFlagRequest     (1U<<31)
...
/* Screen entry in ExtendedDesktopSize / SetDesktopSize screen layouts */
...
/*-----------------------------------------------------------------------------
 * EndOfContinuousUpdates - all continuous updates have been sent (TigerVNC).
 * Also sent unprompted to advertise support for continuous updates.
 */

typedef struct _rfbEndOfContinuousUpdatesMsg {
    CARD8 type;			/* always rfbEndOfContinuousUpdates */
} rfbEndOfContinuousUpdatesMsg;

#define sz_rfbEndOfContinuousUpdatesMsg 1

/*-----------------------------------------------------------------------------
 * Fence - synchronisation request/response (TigerVNC).  Sent in both
 * directions with the same layout (rfbServerFence / rfbClientFence).
 */

typedef struct _rfbFenceMsg {
    CARD8 type;			/* always rfbServerFence/rfbClientFence */
    CARD8 pad1;
    CARD16 pad2;
    CARD32 flags;
    CARD8 length;
    /* followed by char data[length] (max 64 bytes) */
} rfbFenceMsg;
...
typedef union _rfbServerToClientMsg {
    ...
    rfbEndOfContinuousUpdatesMsg eocu;
    rfbFenceMsg fence;
...
/*-----------------------------------------------------------------------------
 * EnableContinuousUpdates - turn continuous updates on or off for the given
 * area (TigerVNC).  The server confirms disabling with EndOfContinuousUpdates.
 */
...
/*-----------------------------------------------------------------------------
 * SetDesktopSize - request a remote framebuffer resize (TigerVNC).  The
 * server replies with an ExtendedDesktopSize pseudo-rect carrying the result.
 */
...
typedef struct _rfbSetDesktopSizeMsg {
    ...
    CARD8 pad2;
    /* followed by numberOfScreens * rfbScreenInfo */
